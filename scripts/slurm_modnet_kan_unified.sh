...
FIXED_SYMBOLIC_N_FEATURES="${FIXED_SYMBOLIC_N_FEATURES:-128}"
...
FIXED_SYMBOLIC_HIDDEN_DIMS="${FIXED_SYMBOLIC_HIDDEN_DIMS:-2}"
FIXED_SYMBOLIC_EDGES_PER_UNIT="${FIXED_SYMBOLIC_EDGES_PER_UNIT:-3}"
FIXED_SYMBOLIC_PRIMITIVES="${FIXED_SYMBOLIC_PRIMITIVES:-zero one identity square cube sin cos tanh exp log1p_abs lorentz gaussian sinh cosh product}"
...
FIXED_SYMBOLIC_PROJECTION_TOP_K="${FIXED_SYMBOLIC_PROJECTION_TOP_K:-2}"
FIXED_SYMBOLIC_GLOBAL_FEATURE_TOP_K="${FIXED_SYMBOLIC_GLOBAL_FEATURE_TOP_K:-5}"
...
if [[ "$RUN_MODE" == "fixed5fold" ]]; then
  ...
  echo "Paper Symbolic-KAN: ${FIXED_SYMBOLIC_N_FEATURES} candidates -> shared top-${FIXED_SYMBOLIC_GLOBAL_FEATURE_TOP_K} -> ${FIXED_SYMBOLIC_HIDDEN_ARRAY[*]} -> sum, edges/unit=${FIXED_SYMBOLIC_EDGES_PER_UNIT}"
  ...
  for task in "${TASKS[@]}"; do
    ...
    conda run --no-capture-output -n "$ENV_NAME" \
      ...
      --folds 0 1 2 3 4 \
      --models symbolic-kan \
      --n-features "$FIXED_SYMBOLIC_N_FEATURES" \
      ...
      --symbolic-global-feature-top-k "$FIXED_SYMBOLIC_GLOBAL_FEATURE_TOP_K" \
      --epochs "$FIXED_EPOCHS" \
      --batch-size "$FIXED_BATCH_SIZE" \
      --val-ratio 0 \
      --early-stopping-monitor loss \
